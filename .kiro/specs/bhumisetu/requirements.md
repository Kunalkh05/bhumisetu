...
**Confirm:** the stage transitions in scope, the baseline choice (statutory period vs. historical percentile vs. fixed threshold), the 90-day horizon, and whether `CENSORED` rows should instead be retained through a survival-style formulation that models time to stage exit with right-censoring rather than discarding the row.
...
*Proposed default:* the presumed governing framework is the Digital Personal Data Protection Act, 2023 together with the rules notified under it and any applicable state land-records retention rules. Every Retention_Period is a Policy_Config value carrying the same state key, act key, and effective-from date as a statutory period. Proposed default Data_Category set and Retention_Periods, each measured from the retention start date defined in Requirement 32: `OWNER_CONTACT` 3 years, `OWNER_IDENTITY` 8 years, `MODEL_FEATURE` 5 years, and `LAND_RECORD`, `DOCUMENT_CONTENT`, and `AUDIT_EVENT` retained without expiry. The erasable Data_Category set is `OWNER_CONTACT`, `OWNER_IDENTITY`, and `MODEL_FEATURE`; the remainder persist as statutory record. A Data_Subject_Request is answered within 30 days of receipt. **Confirm:** the governing framework, the Data_Category set, the erasable subset, the Retention_Period for each Data_Category, the maximum response time for a Data_Subject_Request, and whether a data subject access response may carry the unmasked government identifier notwithstanding the masking rule in Requirement 26.5. The platform cannot run erasure until this is confirmed.
